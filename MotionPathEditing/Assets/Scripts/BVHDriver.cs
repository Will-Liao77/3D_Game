...
public class BVHDriver : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Dictionary<string, Vector3> bonePosStart = new Dictionary<string, Vector3>();
        // Dictionary<string, Vector3> bonePosEnd = new Dictionary<string, Vector3>();
        Vector3[] bonePosStart = new Vector3[bvhHireachy.Count];
        Vector3[] bonePosEnd = new Vector3[bvhHireachy.Count];
        ...
        anim.GetBoneTransform(HumanBodyBones.Hips).position = bvhPos[bp.root.name] * 0.2f;

        foreach (string bname in bvhHireachy.Keys)
        {
            Color color = new Color(1.0f, 0.0f, 0.0f);
            Debug.DrawLine(bvhPos[bname] * 0.2f, bvhPos[bvhHireachy[bname]] * 0.2f, color);
            // bonePosStart.Add(bname, bvhPos[bname] * 0.2f);
            // bonePosEnd.Add(bname, bvhPos[bvhHireachy[bname]] * 0.2f);
        }
        // linerennderer
        //sequence of the bones
        // Chest -> leftUpLeg -> rightUpLeg -> Neck -> leftCollar -> rightCollar -> Head -> lefUpArm -> leftUpArm -> leftLowArm -> leftHand -> rightUpArm -> rightLowArm -> rightHand -> lefyLowLeg -> leftFoot -> rightLowLeg -> rightFoot
        for (int i = 0; i < bvhHireachy.Count; i++)
        {
            Debug.Log("bvhHireachy" + bvhHireachy.ElementAt(i).Key);
        }
    }
}
