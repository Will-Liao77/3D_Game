...
public class BVHAnimationLoader : MonoBehaviour {
    ...
    private Transform getBoneByName(string name, Transform transform, bool first) {
        ...
        // spine1, spine2 ignored
        string Spine = flexibleName("Character1_Spine");

        if (renamingMap.ContainsKey(targetName)) {
            targetName = flexibleName(renamingMap[targetName]);
        }
        //Debug.Log("Looking for bone \"" + targetName);
        if (first) { 
        ...
        for (int i = 0; i < transform.childCount; i++) {
            ...
            if (flexibleName(child.name) == Spine)
            {
                //Debug.Log("Skipping spine child.name \"" + flexibleName(child.name));
                // skip spine and spine1 -> get chest
                Transform secChild = transform.GetChild(i).GetChild(0).GetChild(0);

                if (flexibleName(secChild.name) == targetName)
                {
                    return secChild;
                }
                ...
                // get arm and neck
                for (int j = 0; j < secChild.childCount; j++)
                {
                    Transform thirdChild = secChild.GetChild(j);
                    if (flexibleName(thirdChild.name) == targetName)
                    {
                        return thirdChild;
                    }
                    if (nameMap.ContainsKey(targetName) && nameMap[targetName] == child)
            ...
            //Debug.Log("child.name \"" + flexibleName(child.name));
